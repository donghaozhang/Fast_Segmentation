...
class unet(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        ...
        self.final = nn.Conv2d(in_channels=int(filters[0]), out_channels=int(n_classes), kernel_size=1)
...
class unetUp(nn.Module):
    ...
    def __init__(self, in_size, out_size, is_deconv):
        ...
        if is_deconv:
            ...
            self.up = nn.ConvTranspose2d(in_channels=int(in_size), out_channels=int(out_size), kernel_size=2, stride=2)
        else:
...
class unetConv2(nn.Module):
    ...
    def __init__(self, in_size, out_size, is_batchnorm):
        ...
        if is_batchnorm:
            self.conv1 = nn.Sequential(nn.Conv2d(in_channels=int(in_size), out_channels=int(out_size), kernel_size=3, stride=1, padding=1),
                                       nn.BatchNorm2d(num_features=int(out_size)),
                                       nn.ReLU(),)
            self.conv2 = nn.Sequential(nn.Conv2d(in_channels=int(out_size), out_channels=int(out_size), kernel_size=3, stride=1, padding=1),
                                       nn.BatchNorm2d(num_features=int(out_size)),
                                       nn.ReLU(),)
        else:
            self.conv1 = nn.Sequential(nn.Conv2d(in_channels=int(in_size), out_channels=int(out_size), kernel_size=3, stride=1, padding=1),
