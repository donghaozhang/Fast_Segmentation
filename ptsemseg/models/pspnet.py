...
class pspnet(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True):
        ...
        # Final Classifier
        self.finaldeconvbnrelu1 = nn.Sequential(nn.ConvTranspose2d(in_channels=int(filters[0]), out_channels=int(32/feature_scale), kernel_size=3, stride=2, padding=1),
                                      nn.BatchNorm2d(num_features=int(32/feature_scale)),
        ...
        self.finalconv3 = nn.Conv2d(in_channels=int(32/feature_scale), out_channels=int(n_classes), kernel_size=2, stride=2, padding=0)

    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(nn.Conv2d(in_channels=int(self.inplanes), out_channels=int(planes * block.expansion),
                                                 kernel_size=1, stride=stride, bias=False),
                                       nn.BatchNorm2d(num_features=int(planes * block.expansion)),)
        layers = []
