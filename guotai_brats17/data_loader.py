	...
	def load_data(self):
		...
		for i in range(data_num):
			...
			for mod_idx in range(len(self.modality_postfix)):
				...
				if (mod_idx == 0):
					margin = 5
					log('The shape of current volume is {}'.format(volume.shape))
					bbmin, bbmax = get_ND_bounding_box(volume, margin)
					log('bbmin is {} and bbmax is {}'.format(bbmin, bbmax))
					volume_size = volume.shape
				log('The shape of volume before cropping is {}'.format(volume.shape))
				volume = crop_ND_volume_with_bounding_box(volume, bbmin, bbmax)
				log('The shape of volume after cropping is {}'.format(volume.shape))
				if (self.data_resize):
					log('data_resize if is being called')
				...
				# volume contains t1 t1ce t2 flair
				volume_name_list.append(volume_name)
				# print('the volume name is ', volume_name)
	...
	def __get_one_batch(self):
		...
		for i in range(batch_size):
			...
			if (self.with_ground_truth):
				...
				if (train_with_roi_patch):
					log('train_with_roi_patch is being called')
			...
			log('This is the line 234')
			log('The shape of data_volumes is {}'.format(data_volumes[0].shape))
			log('The shape of data_volumes is {}'.format(data_volumes[1].shape))
			...
			log('sub_data_shape is : {}'.format(sub_data_shape))
			sub_label_shape = [label_slice_number, label_shape[1], label_shape[2]]
			log('The curren volume shape is {}'.format(volume_shape))
			log('sub_label_shape is {}'.format(sub_label_shape))
			log('The boundingbox is {}'.format(boundingbox))
			log('The shape of batch_sample_model is {}'.format(batch_sample_model))
			...
			if (flip):
				log('The Flip if is being called')
				sub_weight = np.flip(sub_weight, -1)
			if (down_sample_rate != 1.0):
				log('The down_sample_rate is being called')
